...
def calculate_ndcg(
    ...
    Args:
        predicted_scores: [batch_size, num_items] 예측 점수 (retrieval service에서 받은 scores)
...
class RecRewardFrunction:
    ...
    def __call__(
        ...
        **kwargs
        ...
        Args:
            ...
            target_items: [batch_size] 타겟 아이템 ID
            history_items: [batch_size, *] 히스토리 아이템 ID
        ...
        scores = ray.get(scores_ref)  # [batch_size, num_items]
        
        # 2. 리워드 타입에 따라 계산
        if self.reward_type == "ndcg":
            rewards = calculate_ndcg(scores, targets, histories, k=self.k)
        elif self.reward_type == "hit":
            rewards = calculate_hit_rate(scores, targets, histories, k=self.k)
        elif self.reward_type == "mrr":
            rewards = calculate_mrr(scores, targets, histories, k=self.k)
        elif self.reward_type == "mixed":
            # NDCG + Hit@K의 가중 평균
            ndcg = calculate_ndcg(scores, targets, histories, k=self.k)
            hit = calculate_hit_rate(scores, targets, histories, k=self.k)
    ...
    def compute_metrics(
        ...
        ndcg = calculate_ndcg(scores, targets, histories, k=self.k)
        hit = calculate_hit_rate(scores, targets, histories, k=self.k)
        mrr = calculate_mrr(scores, targets, histories, k=self.k)
